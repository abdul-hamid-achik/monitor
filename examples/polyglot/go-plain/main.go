...
func tickErrors() {
	for _, p := range []string{`{"ok":true}`, "bad-payload-123", `{"ok":true}`} {
		if _, err := flakyParse(p); err != nil {
			fmt.Fprintf(os.Stderr, "[go-plain] caught in tickErrors: %v\n", err)
		}
	}
}
...
func main() {
	pid := os.Getpid()
	fmt.Fprintf(os.Stderr, "[go-plain] pid=%d workload starting (no pprof)\n", pid)
	...
	close(stop)
	panic(fmt.Sprintf("go-plain workload: intentional uncaught failure at t=%.1fs", time.Since(start).Seconds()))
}
