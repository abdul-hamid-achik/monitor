...
// like <tmp>/acme/web-api/package.json, with .git at <tmp>/acme,
// must give project=acme service=web-api from both call sites.
//
...
// investigate gave project=acme/service=web-api for the SAME pid).
...
func TestWatchAndInvestigateAgreeOnProject(t *testing.T) {
	...
	if watchIssue.Project != "acme" || watchIssue.Service != "web-api" {
		t.Fatalf("watch identity = project=%q service=%q, want project=acme service=web-api", watchIssue.Project, watchIssue.Service)
	}
	if investigateIssue.Project != "acme" || investigateIssue.Service != "web-api" {
		t.Fatalf("investigate identity = project=%q service=%q, want project=acme service=web-api", investigateIssue.Project, investigateIssue.Service)
	}
